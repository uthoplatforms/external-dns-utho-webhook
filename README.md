...
your domains.
For detailed technical instructions on how the Utho webhook is deployed using the Bitnami Helm charts for ExternalDNS,
...
First, create the Utho secret:

```yaml
kubectl create secret generic utho-credentials --from-literal=api-key='<EXAMPLE_PLEASE_REPLACE>' -n external-dns
```
...
provider:
  ...
  webhook:
    image:
      repository: utho/external-dns-utho-webhook
      tag: 0.1.2
    env:
      - name: UTHO_API_KEY
        valueFrom:
...
```shell
# install external-dns with helm
helm install external-dns-utho external-dns/external-dns -f external-dns-utho-values.yaml --version 1.14.3 -n external-dns
```
...
- if your records don't get deleted when applications are uninstalled, you
  ...
  processed. Please add the following to `external-dns-utho-values.yaml` if
